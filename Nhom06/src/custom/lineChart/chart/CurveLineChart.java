package custom.lineChart.chart;
...
import custom.lineChart.chart.blankchart.BlankPlotChart;
import custom.lineChart.chart.blankchart.BlankPlotChatRender;
import custom.lineChart.spline.Spline;
import custom.lineChart.spline.SplinePoint;
...
public class CurveLineChart extends JComponent {
    ...
    private void drawLabel(Graphics2D g2, SplinePoint s) {
        ...
        if (selectedIndex >= 0) {
            String text = blankPlotChart.getFormat().format(model.get(selectedIndex).getValues().get(index));
    ...
    private SplinePoint[] toPoint(Rectangle2D rec, int index, double maxValue) {
        ...
        for (int i = 0; i < model.size(); i++) {
            points[i + 1] = toPoint(rec, i, model.size(), model.get(i).getValues().get(index), maxValue);
        }
        points[0] = points[1];
